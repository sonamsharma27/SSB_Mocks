            ...
            feedback.map((ans,index)=>(
              <div className="ppdtfeedback container border border-1" key={ans._id}>
                 <div>
                  ...
                  {
                  ans.situation.split("|||").map((item,index)=><li key={index} className={item.length?"srtlistitems":"d-none"}>{item}</li>
                  )}
                  </div>
                  
                 </div>
                  <p style={{color: "black",fontSize: "20px"}} className='mb-0 mt-2'>Responses: </p>
                  <p>{ans.result}</p>
